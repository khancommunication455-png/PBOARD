...
interface StyleKitUserDictionaryDao {
    ...
    fun suggestByPrefix(prefix: String, limit: Int = 5): List<StyleKitUserDictionaryEntity>

    @Query("SELECT * FROM sk_user_dictionary ORDER BY added_at DESC")
    fun observeAll(): Flow<List<StyleKitUserDictionaryEntity>>

    @Query("SELECT * FROM sk_user_dictionary ORDER BY added_at DESC")
    fun getAll(): List<StyleKitUserDictionaryEntity>

    @Query("SELECT * FROM sk_user_dictionary WHERE word = :word LIMIT 1")
    fun getByWord(word: String): StyleKitUserDictionaryEntity?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    fun insert(entity: StyleKitUserDictionaryEntity): Long

    @Query("DELETE FROM sk_user_dictionary WHERE word = :word")
    fun deleteByWord(word: String): Int
