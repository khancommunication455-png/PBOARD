...
interface AutoSenderDao {
    ...
    @Query("SELECT * FROM sk_auto_sender_script ORDER BY updated_at DESC")
    fun getAllScripts(): List<AutoSenderScriptEntity>

    @Query("SELECT * FROM sk_auto_sender_script WHERE row_id = :id LIMIT 1")
    fun getScriptById(id: Long): AutoSenderScriptEntity?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    fun insertScript(entity: AutoSenderScriptEntity): Long

    @Update
    fun updateScript(entity: AutoSenderScriptEntity)

    @Delete
    fun deleteScript(entity: AutoSenderScriptEntity)

    @Query("DELETE FROM sk_auto_sender_script WHERE row_id = :id")
    fun deleteScriptById(id: Long): Int
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    fun insertLog(entity: AutoSenderLogEntity): Long
