...
interface TrigramDao {
    ...
    fun suggestNext(first: String, second: String, limit: Int = 3): List<TrigramEntity>

    @Query("SELECT * FROM sk_trigram WHERE first_word = :first AND second_word = :second AND third_word = :third LIMIT 1")
    fun getExact(first: String, second: String, third: String): TrigramEntity?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    fun insert(entity: TrigramEntity): Long
    ...
    fun bumpFrequency(first: String, second: String, third: String, delta: Int, now: Long): Int
