...
interface BigramDao {
    ...
    fun suggestNext(first: String, limit: Int = 5): List<BigramEntity>

    @Query("SELECT * FROM sk_bigram WHERE first_word = :first AND second_word = :second LIMIT 1")
    fun getExact(first: String, second: String): BigramEntity?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    fun insert(entity: BigramEntity): Long

    @Query("UPDATE sk_bigram SET frequency = frequency + :delta, last_used = :now WHERE first_word = :first AND second_word = :second")
    fun bumpFrequency(first: String, second: String, delta: Int, now: Long): Int
