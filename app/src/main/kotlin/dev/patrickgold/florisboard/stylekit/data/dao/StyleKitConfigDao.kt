...
interface StyleKitConfigDao {
    ...
    @Query("SELECT * FROM sk_config WHERE id = 1 LIMIT 1")
    fun get(): StyleKitConfigEntity?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    fun upsert(entity: StyleKitConfigEntity)

    @Update
    fun update(entity: StyleKitConfigEntity)
    ...
    fun setPreset(id: Long, enabled: Boolean)

    @Query("UPDATE sk_config SET emoji_shortcuts_enabled = :enabled WHERE id = 1")
    fun setEmojiShortcuts(enabled: Boolean)
}
