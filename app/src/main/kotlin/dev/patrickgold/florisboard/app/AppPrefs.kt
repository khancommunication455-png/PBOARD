...
abstract class FlorisPreferenceModel : PreferenceModel() {
    ...
    inner class Suggestion {
        ...
        // --- StyleKit: on-device personalized learning (Part 1) ---
        /**
         * Master switch for personalized, on-device learning. When false, the
         * [dev.patrickgold.florisboard.stylekit.nlp.AdaptiveLearningProvider] returns
         * no candidates and never writes to the frequency/bigram/trigram tables.
         *
         * Distinct from `incognitoMode` (which is per-session): this is the global
         * opt-out. Both must be off (i.e. incognito off + personalized learning on)
         * for the pipeline to actually train.
         */
        val personalizedLearning = boolean(
            key = "suggestion__personalized_learning",
            default = true,
        )

        /**
         * Whether the merged toolbar/suggestion-row auto-swap (Gboard-style) is
         * active. When true and `smartbar.layout` is set to the new
         * `SUGGESTIONS_ACTIONS_AUTO` value, the smartbar's center content
         * crossfades between the toolbar icon row and 3 suggestion chips based
         * on whether candidates are available.
         */
        val autoSwapToolbarAndSuggestions = boolean(
            key = "suggestion__auto_swap_toolbar_and_suggestions",
            default = true,
        )
    }
